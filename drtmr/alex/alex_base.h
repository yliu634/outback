...
inline int tzcnt_u64(uint64_t value) {
    int count = 0;
    const int numBits = sizeof(uint64_t) * 8; // 总位数，通常是 64

    // 检查每一位，直到找到一个非零位
    while ((value & 1) == 0 && count < numBits) {
        ++count;
        value >>= 1; // 右移 value，检查下一位
    }

    return count;
}

inline int lzcnt_u64 (uint64_t value) {
    int count = 0;
    const int numBits = sizeof(uint64_t) * 8; // 总位数，通常是 64
    uint64_t mask = 1ULL << (numBits - 1); // 设置最高位为 1，其余位为 0
