...
public:
   ...
   *        * leaf_num in the second one would be align up to see the minimal one page can hold x leafs
  ...
  auto get_leaf(u64 num) -> char * {
    ...
    //return mem_pool + 2*sizeof(u64) + num*S;    // why there is a 2 here
    return mem_pool + num*S;  // bucket
  }
...
private:
  ...
  void prealloc_leaves(u64 leaf_num) {
    ...
    for(int i=0; i<leaf_num; i++) {
      ...
      //memcpy(mem_pool+cur_alloc_sz, cur_leaf, sizeof(Leaf));
      //cur_alloc_sz += sizeof(leaf_t);
    }
    LOG(3) << "Preallocate leaf number--> [used, total]:" << cur_alloc_sz/sizeof(Leaf);
  }
